...
import { SUBJECTS, TRANSVERSAL_COMPETENCIES } from '../constants';
...
const CurriculumResults: React.FC<CurriculumResultsProps> = ({ 
  ...
  const renderSection = (
    title: string, 
    items: { code: string; text: string; subjectId?: string }[] | undefined, 
    ...
    return (
      <div className="mb-4 flex flex-col h-full animate-fade-in">
        <div className="flex justify-between items-center mb-3 px-2">
            <h4 className={`text-[10px] font-black uppercase tracking-[0.15em] text-${defaultColor}-700`}>{title}</h4>
            {onLoadMore && (
                <button 
        ...
        <div className="space-y-2.5 flex-grow">
          {safeItems.map((item, idx) => {
            ...
            const fullItem: CurriculumItem = { id: itemId, code: item.code, text: item.text, type, subjectId: item.subjectId };
            ...
            return (
              <div 
                ...
                className={`
                  ...
                  ${isSelected ? `bg-${areaColor}-50 border-${areaColor}-200 shadow-sm` : 'bg-white border-slate-100 hover:border-slate-200 hover:shadow-md'}
                `}
              >
                <button
                ...
                <div className="flex items-start gap-3">
                  ...
                  `}>
                    {isSelected && <Check size={12} className={`text-${areaColor}-600`} strokeWidth={4} />}
                    {!isSelected && <Plus size={12} className="text-slate-300 opacity-0 group-hover:opacity-100" />}
                  </div>
                  <div className="flex-grow">
                    <span className={`block text-[9px] font-black uppercase bg-${areaColor}-100 text-${areaColor}-700 px-2 py-0.5 rounded-md mb-1 w-fit`}>
                      {typeLabelMap[type]}
                    </span>
                    <span className={`font-black text-[9px] block mb-0.5 tracking-widest opacity-70 ${isSelected ? `text-${areaColor}-700` : 'text-slate-500'}`}>
                      {item.code}
                    </span>
  ...
  return (
    <div className="mt-6 animate-fade-in">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {renderSection("Competències", localCompetencies, 'competencia', 'blue')}
        {renderSection("Criteris d'Avaluació", localCriteria, 'criteri', 'sky')}
        {renderSection("Sabers i Continguts", localSabers, 'saber', 'indigo')}
      </div>
    </div>
  );
};
