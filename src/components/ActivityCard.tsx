...
import { SUBJECT_ICONS, COLOR_MAP } from '../constants'; // Import SUBJECT_ICONS and COLOR_MAP from constants
...
const ActivityCard: React.FC<ActivityCardProps> = ({ activity, activitySubjectIds, onEdit, onCopy, onView }) => {
  // Use COLOR_MAP for reliable Tailwind classes
  const colors = COLOR_MAP[activity.color || 'blue'] || COLOR_MAP.blue;
  const colorClass = colors.bg;
  const textClass = colors.text;
  const lightBgClass = colors.lightBg; 
