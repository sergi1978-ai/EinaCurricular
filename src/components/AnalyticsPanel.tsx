...
import { SUBJECTS, TRANSVERSAL_COMPETENCIES, SCHOOL_YEARS, GRADES, SUBJECT_ICONS, COLOR_MAP } from '../constants';
import { 
  TrendingUp, 
  AlertCircle, 
  Target, 
...
const AnalyticsPanel: React.FC<AnalyticsPanelProps> = ({ activities }) => {
  ...
  const getAreaColors = (subjectId?: string) => {
    if (!subjectId) return COLOR_MAP.blue;
    const area = allSubjects.find(s => s.id === subjectId);
    return COLOR_MAP[area?.color || 'blue'] || COLOR_MAP.blue;
  };
  ...
  const renderRanking = (data: any[], typeLabel: string) => {
    ...
    return (
      <div className="grid grid-cols-1 gap-6 animate-fade-in">
        {top.map((item, i) => {
          const colors = getAreaColors(item.subjectId);
          ...
          return (
            <div key={item.code} className="group bg-white border border-slate-100 rounded-[2rem] p-6 hover:border-blue-200 hover:shadow-xl transition-all duration-300 flex flex-col md:flex-row gap-8 items-center overflow-hidden relative">
              <div className={`absolute left-0 top-0 h-full w-1.5 ${colors.bg}`}></div>
              
              <div className="shrink-0 flex items-center gap-6">
                <div className={`w-12 h-12 rounded-xl ${colors.bg} text-white flex items-center justify-center font-black text-lg shadow-md group-hover:scale-110 transition-transform`}>
                  {i + 1}
                </div>
                <div className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex flex-col items-center min-w-[70px]">
                  ...
                  <span className={`text-sm font-black ${colors.text}`}>{item.percentage}%</span>
                </div>
              </div>

              <div className="flex-grow space-y-3">
                <div className="flex flex-wrap items-center gap-3">
                  <span className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${colors.lightBg} ${colors.text} ${colors.border}`}>
                    {typeLabel}
                  </span>
          ...
          <div className="md:col-span-8 space-y-4">
            ...
            <div className="space-y-4">
              {areaStats.map(([id, count]) => {
                ...
                const colors = COLOR_MAP[sub?.color || 'blue'] || COLOR_MAP.blue;
                  ...
                  >
                    <div className="flex justify-between items-center mb-2 px-2">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg ${colors.bg} text-white flex items-center justify-center shadow-sm`}>
                    ...
                    <div className="h-5 w-full bg-slate-100 rounded-xl overflow-hidden border border-slate-50 relative">
                      <div 
                        className={`h-full ${colors.bg} transition-all duration-1000 shadow-md relative`}
  ...
  return (
    <div className="max-w-[1400px] mx-auto py-10 animate-fade-in space-y-10 px-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          { label: 'Elements Curriculars', value: stats.competencies.length + stats.criteris.length + stats.sabers.length, icon: Layers, colors: COLOR_MAP.blue },
          { label: 'Freq. Mitjana Sabers', value: stats.total > 0 ? (stats.sabers.reduce((acc, curr) => acc + (curr.count || 0), 0) / (stats.sabers.length || 1)).toFixed(1) : 0, icon: TrendingUp, colors: COLOR_MAP.indigo },
          { label: 'Criteris d\'Avaluació', value: stats.criteris.length, icon: CheckCircle, colors: COLOR_MAP.cyan },
          { label: 'Competències Espec.', value: stats.competencies.length, icon: Target, colors: COLOR_MAP.emerald }
        ].map((kpi, i) => (
          <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:-translate-y-1 transition-all">
            <div className={`p-4 ${kpi.colors.lightBg} ${kpi.colors.text} rounded-2xl mb-4 group-hover:scale-110 transition-transform`}>
              <kpi.icon size={24} />
            </div>
